...
| `browser_fetch.py` | `BrowserFetcher` — `BrowserManager` + `page.content()` + navigation status/timeout |
...
- Always return transport facts (`final_url` after redirects, method, status code, HTML body). Provider detection belongs in pipeline extractor code, not in `core.fetch`.
