...
def _write_artifacts(
    ...
    extracted = write_extracted_job_markdown(listing, run_id=_RUN_ID)
    analysis_path: Path | None = None
    analysis = analyses.get(listing.dedup_key)
    if analysis is not None:
        analysis_path = write_job_analysis_markdown(listing, analysis, run_id=_RUN_ID)
...
async def main() -> None:
    ...
    if isinstance(out, dict):
        ...
        listing = qualified[0]
        print("Phase:", phase)
        extracted, analysis_path = _write_artifacts(listing, analyses)
    ...
    extracted, analysis_path = _write_artifacts(listing, out.job_analyses)
    _print_success(listing, extracted, analysis_path)
