...
Every interactive action taken by a provider adapter MUST go through this
...
# QWERTY adjacency map for typo simulation (most common neighbor per key).
...
class HumanBehavior:
    ...
    async def between_pages_pause(self, source: JobSource) -> None:
        """Extra jitter after DomainRateLimiter.wait() to break timing patterns."""
        jitter = random.uniform(0.5, 2.5)
        logger.debug("between_pages_jitter", source=source.value, jitter_s=round(jitter, 2))
