"""Failure-capture utilities for discovery diagnostics.

When enabled, captures contextual artifacts (metadata JSON, screenshot, HTML)
for provider failures so runs are debuggable without interactive browser UI.
"""
...
class FailureCapture:
    """Persist diagnostics for discovery failures."""
    ...
    def _event_dir(self, source: JobSource, stage: str) -> Path:
        self._counter += 1
        stamp = datetime.now(UTC).strftime("%Y%m%dT%H%M%S.%fZ")
        event_name = f"{self._counter:04d}-{_safe_name(stage)}-{stamp}"
        path = self._base_dir / self._run_id / source.value / event_name
